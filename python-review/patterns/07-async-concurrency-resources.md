...
Agents write async-shaped sync code and leak resources.

## Smells

`async def` with `requests`; CPU work in event loop; unbounded `gather`; sessions shared across threads; clients not closed.
...
> Is this concurrency model explicit or accidental?
